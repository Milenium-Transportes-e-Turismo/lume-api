...
| Catálogo de agentes em banco novo | Corrigido no código: além do backfill da migration `20260829000200`, o `ProductionBootstrapService` sincroniza o catálogo idempotente dentro da mesma transação, depois de criar ou atualizar a empresa. | Executar `tenant:bootstrap` no banco descartável e repetir o comando para validar a operação real. Não inserir registros manualmente. | Exatamente sete agentes, sete runtimes e sete referências de credencial distintas para o tenant, sem duplicações após a repetição. |
...
| Compose de produção | O volume privado de Knowledge e `KNOWLEDGE_STORAGE_PATH` já são injetados, preparados e montados pelo `compose.prod.yml`. Continuam não encaminhados pelo Compose `TENANT_API_PUBLIC_URL`, as sete chaves dos agentes e o timeout do runtime; a ativação de WhatsApp também depende das configurações externas exigidas pelo ambiente. | Completar o encaminhamento dessas variáveis e validar o Compose com o inventário de produção e os segredos reais no orquestrador autorizado. | API inicia com `NODE_ENV=production`, readiness saudável, canal provisionável e persistência de Knowledge após recriação do container. |
